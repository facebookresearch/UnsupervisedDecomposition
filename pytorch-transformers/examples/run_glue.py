...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        logger.info("  Data Sampler = %s", eval_sampler.__class__.__name__)
        ...
        if args.output_mode == "classification":
            logits_file = os.path.join(args.data_dir, "logits.tsv")
            with open(logits_file, "w") as writer:
                writer.write('\t'.join([str(i) for i in range(model.num_labels)]) + '\n')
                for ex_logits in preds:
                    writer.write('\t'.join([str(logit) for logit in ex_logits]) + '\n')
